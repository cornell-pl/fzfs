...
val x_parse_with_error :
  ((Lexing.lexbuf -> ForestParser.token) -> Lexing.lexbuf -> (ForestTypes.var * 'a ForestTypes.ast) list) ->
  (Lexing.lexbuf -> ForestParser.token) ->
  Lexing.lexbuf ->
  (ForestTypes.var * 'a ForestTypes.ast) list

(** [x_parse_with_error] takes a parse function, a lexer, and a lexbuf and parses. *)

val x_parse_string : (Lexing.lexbuf -> (ForestTypes.var * 'a) list) -> Location.t -> string -> (ForestTypes.var * 'a) list
...
val forest_parse_with_error : Lexing.lexbuf -> (ForestTypes.var * ForestTypes.specification ForestTypes.ast) list
val forest_parse_string : Location.t -> string -> (ForestTypes.var * ForestTypes.specification ForestTypes.ast) list
